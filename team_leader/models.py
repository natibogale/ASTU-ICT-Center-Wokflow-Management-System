...
class ExpertReportMessages(models.Model):
    id = models.AutoField(primary_key=True)
    reportId = models.ForeignKey(Reports ,on_delete=models.CASCADE, max_length=500, verbose_name="Report ID")
    reportUnique = models.ForeignKey('director.Reports',to_field='expertUnique',on_delete=models.CASCADE,related_name='+', max_length=500, blank=True, null=True)
    messageSender = models.ForeignKey('authentication.User' ,related_name="+", on_delete=models.CASCADE ,blank=True,null=True, max_length=200, verbose_name="Message From")
    ...
    def __str__(self):
        return f'{self.id}'
    class Meta:
        verbose_name_plural = 'Expert Report Messages'
